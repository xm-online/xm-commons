...
import com.icthh.xm.commons.errors.exception.NoContentException;
import com.icthh.xm.commons.errors.vm.ErrorVM;
import com.icthh.xm.commons.errors.vm.FieldErrorVM;
import com.icthh.xm.commons.errors.vm.ParameterizedErrorVM;
...
public class ExceptionTranslator {
    ...
    @ResponseStatus(HttpStatus.BAD_REQUEST)
    @ResponseBody
    public FieldErrorVM processValidationError(MethodArgumentNotValidException ex) {
        ...
        for (ObjectError error: errors) {
            dto.getParams().put(error.getObjectName(),error.getDefaultMessage());
        }
        return dto;
    }
    ...
    public ResponseEntity<ErrorVM> processHttpServerError(HttpServerErrorException ex) {
        ...
        if (responseStatus != null) {
            ...
            fieldErrorVM = new ErrorVM(ERROR_PREFIX + responseStatus.value(), translate(ERROR_PREFIX + responseStatus.value()));
        } else {
    ...
    public ParameterizedErrorVM processParameterizedValidationError(BusinessException ex) {
        return new ParameterizedErrorVM(ex.getCode() == null ? ErrorConstants.ERR_BUSINESS : ex.getCode(),
                ex.getCode() == null ? ex.getMessage() : translate(ex.getCode()), ex.getParamMap());
    }
