...
public class DynamicTopicConsumerConfiguration extends AbstractDynamicConsumerConfiguration {
    ...
    @Value("${application.scheduler-config.build-topic-consumers:false}")
    private boolean buildTopicConsumers;

    @Value("${application.scheduler-config.build-multi-microservice-consumers:false}")
